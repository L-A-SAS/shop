...
<div class="products content-wrapper">
    ...
    <h1>חנות כדורי פלא</h1>

    <div class="products-header">
        <p><?=$total_products?> מוצרים</p>
        <form action="" method="get" class="products-form">
            <label class="category"  style="text-align:right;">
                סוג
                <br>
            <input type="hidden" name="page" value="products">
            
                <select name="category">
                    <option value="all"<?=($category == 'all' ? ' selected' : '')?>>הכל</option>
                    <?php foreach ($categories as $c): ?>
                    <option value="<?=$c['id']?>"<?=($category == $c['id'] ? ' selected' : '')?>><?=$c['name']?></option>
                    <?php endforeach; ?>
                </select>
            </label>
            <label class="sortby" style="text-align:right;">
                סדר לפי
                <br>
                <select name="sort">
                    <option value="sort1"<?=($sort == 'sort1' ? ' selected' : '')?>>א-ת</option>
                    <option value="sort2"<?=($sort == 'sort2' ? ' selected' : '')?>>ת-א</option>
                    <option value="sort3"<?=($sort == 'sort3' ? ' selected' : '')?>>הכי חדש</option>
                    <option value="sort4"<?=($sort == 'sort4' ? ' selected' : '')?>>הכי ישן</option>
                    <option value="sort5"<?=($sort == 'sort5' ? ' selected' : '')?>>מחיר גבוה</option>
                    <option value="sort6"<?=($sort == 'sort6' ? ' selected' : '')?>>מחיר נמוך</option>
                </select>
            </label>
        </form>
    </div>
    ...
    <div class="buttons">
        ...
        <a href="<?=url('index.php?page=products&p=' . ($current_page-1) . '&category=' . $category . '&sort=' . $sort)?>">הבא</a>
        <?php endif; ?>
        <?php if ($total_products > ($current_page * $num_products_on_each_page) - $num_products_on_each_page + count($products)): ?>
        <a href="<?=url('index.php?page=products&p=' . ($current_page+1) . '&category=' . $category . '&sort=' . $sort)?>">הקודם</a>
        <?php endif; ?>
    </div>
